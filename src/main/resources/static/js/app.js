...
function switchTab(n, el) {
  ...
  if (n === 10 && !Auth.canAccessMenu('公告管理')) {
    alert('权限不足，无法访问公告管理');
    return;
  }
  // 数据看板权限检查
  ...
  var titles = ['工作台', '考勤日历', '数据看板', '审批流', '通知中心', '会议预约', '请假申请', '加班申请', '我的申请', '公告列表', '公告管理'];
  ...
  if (n === 9) loadAnnouncementList();
  if (n === 10) loadAnnounceManageList();
  // 切换面板时刷新未读数量Badge
  refreshUnreadBadge();
}
